  ...
  try {
    ...
    const newUser = await User.findOne(
      ...
      return res.status(201).json(
        new ApiResponse(201, 'User registered successfully ><', { user: newUser})
      )
  } catch (error) {
...
const verifyEmail = asyncHandler(async (req, res) => {    
    try {
      ...
      return res.status(200).json(
         new ApiResponse(200, 'user verified', user)
      )
    }else{
      ...
      console.log(error);
      return res.status(400).json(
        new ApiResponse(400, 'Couldn\'t verify user', error)
      )
    }
...
const resendEmailVerification = asyncHandler(async (req, res) => {
  ...
  try {
      ...
      return res.json(
        new ApiResponse(200, 'Email sent', sentEmail)
      )
    } catch (error) {
      user.emailVerificationToken = undefined;
      user.emailVerificationExpiry = undefined;
...
const forgotPasswordRequest = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(
      new ApiResponse(200, 'Email to reset password sent successfully.', sentEmail)
    )
  } catch (error) {
    console.log(error);
    if(error instanceof ApiError) throw error;

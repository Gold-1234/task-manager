...
const getProjects = async (req, res) => {
  ...
  const projects = await ProjectMember.findOne({
    user: user._id
  });
  if (!projects) {
    throw new ApiError(404, "Projects not found.");
...
const createProject = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(200, "Project successfully created : )", [
          project,
          projectMember,
        ]),
      );
...
const addMemberToProject = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Members added successfully", member));
  } catch (error) {
...
const deleteMember = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Member deleted successfully", member));
  } catch (error) {
...
const updateMemberRole = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Role updated successfully", update));
  } catch (error) {
    console.log(error);
